...
The table separates executable backend artifacts from design-time artifacts
for the planned LLM interaction layer. Current entries include CLI surfaces,
generated outputs, and the identity, prompt, capability, tool-boundary, and
...
to source collections, generated outputs, future run records, and
...
The current `agent/` files describe the planned LLM interaction layer at design
time and point to the deterministic CLI as its action surface. The CLI does not
load them as runtime instructions. They remain design-time artifacts until an
explicitly scoped runtime wrapper implements them.
...
- #52 - [optional runtime wrapper prototype](runtime-wrapper-prototype.md)
