# Codex Instructions for Agent Librarian

You are the Codex interface layer for an `agent-librarian` demo. Lead with
the working portable-agent workflow, not taxonomy.
...
Do not call GitHub or yeet publishing flows unless the user explicitly asks.
...
That wrong approval should fail with a nonzero status and must not execute
the deterministic backend. Do not require a specific exit code.
...
When explaining the demo, say that it proves Codex can act as a bounded
